...
map<int,pair<int,int> > signal_address;
...
long long int *d_M_EVENT_time;
int *d_M_EVENT_val;

long long int max_part_number;
int thread_number, sig_part_number;
int gpu_size;
...
int total_number_of_gates, total_number_of_ports, total_number_of_partitions, total_number_of_port_per_parts, total_sum_of_sdf, total_number_of_signal_parts, total_input_sig;

int *h_sig_input_M_pointer, *d_sig_input_M_pointer;
int *h_sig_input_D_pointer, *d_sig_input_D_pointer;
int *h_sig_input_size, *d_sig_input_size;
long long int *h_input_time, *d_input_time;
int *h_input_val, *d_input_val;

int *h_gate_start, *d_gate_start;
int *h_gate_end, *d_gate_end;
int *h_port_start, *d_port_start;
int *h_port_end, *d_port_end;

int *h_gate_ind, *d_gate_ind;
int *h_gate_partition_number, *d_gate_partition_number;
int *d_out_pointer;

int *h_gate_partition_index, *d_gate_partition_index;
int *d_port_partition_start;
int *d_port_partition_end;
int *d_partition_pre;
int *d_partition_address_first, *d_partition_address_last;
bool *d_zero_time;
int *h_partition_number, *d_partition_number;
int *h_stimuli_sdf_first, *d_stimuli_sdf_first;
int *h_stimuli_sdf_end, *d_stimuli_sdf_end;
int *h_table_id, *d_table_id;
int *h_table, *d_table;
long long int *d_tmp_memory_time;
int *h_partition_output_address, *d_partition_output_address;
long long int *d_partition_min_time;
int *h_stimuli_size, *d_stimuli_size;
int *h_sdf_mask, *d_sdf_mask;
int *h_sdf_s_in, *d_sdf_s_in;
int8_t *h_t_edge , *d_t_edge;
int *h_signal_pointer_first, *d_signal_pointer_first;
int *h_signal_pointer_last, *d_signal_pointer_last;
bool *h_signal_ending_part, *d_signal_ending_part;
long long int *h_saif_part, *d_saif_part;
bool *h_fake_signal, *d_fake_signal;
bool *h_fake_part, *d_fake_part;


void input_init(){

	cudaMalloc(&d_M_EVENT_time, gpu_size*sizeof(long long int));
	...
	h_input_time = (long long int *)malloc(total_event*sizeof(long long int));
	...
	cudaMalloc(&d_input_time, total_event*sizeof(long long int));
	...
	cudaMemcpy(d_input_time, h_input_time,total_event*sizeof(long long int), cudaMemcpyHostToDevice);
...
__global__ void kernel0_input_init(int sig_part_number,int sig_n, int *d_sig_input_M_pointer, int *d_sig_input_D_pointer, int *d_sig_input_size, long long int *d_input_time, int *d_input_val, long long int *d_M_EVENT_time, int *d_M_EVENT_val){
...
vector<int> gate_partition_vec;
vector<long long int> gate_size_vec;
long long int total_size_of_inputs;
...
void memory_allocate(){
	...
	cudaMalloc(&d_tmp_memory_time, total_number_of_partitions * 12 * sizeof(long long int));
	...
	cudaMalloc(&d_partition_min_time, total_number_of_partitions*sizeof(long long int));
	...
	h_saif_part = (long long int *)malloc(total_number_of_signal_parts*3*sizeof(long long int));
	cudaMalloc(&d_saif_part, total_number_of_signal_parts*3*sizeof(long long int));
...
__global__ void kernel_print_waveform(int ptr1, int ptr2, int *d_M_EVENT_val, long long int *d_M_EVENT_time){
...
__device__ int check_port_index(long long int Time, int first_pointer, int last_pointer, long long int *d_M_EVENT_time){
	int l = first_pointer;
	int r = last_pointer ;
	while(l < r - 1){
		int mid = (r + l) / 2;
...
__device__ bool check_time(long long int Time, long long int tmp_size, int p1, int p2, int *portfirst, int *portlast, long long int *d_M_EVENT_time){
	long long int ans = 0;
	for(int i = p1 ; i < p2 ; i++){
		int index = check_port_index(Time, portfirst[i - p1], portlast[i - p1], d_M_EVENT_time);
...
__device__ long long int find_time(long long int tmp_size, int p1, int p2, int *portfirst, int *portlast, long long int *d_M_EVENT_time){

	long long int l = 0;
	long long int r = 1000000LL*1000000LL;
	while(l < r - 1){
		long long int mid = (r + l) / 2;
		if(check_time(mid, tmp_size, p1, p2, portfirst, portlast, d_M_EVENT_time)){
			r = mid;
		}
		else{
			l = mid;
	...
	if(check_time(l, tmp_size, p1, p2, portfirst, portlast, d_M_EVENT_time)){
		return l;
	}
	return r;
}

__global__ void kernel1_find_parttion_bounds(int part_n, int *d_gate_partition_number, int *d_partition_number, int *d_gate_ind, bool *d_zero_time, int *d_partition_pre, int *d_partition_address_first, int *d_partition_address_last, int *d_port_partition_start, int *d_port_partition_end, int *d_gate_partition_index, int *d_gate_start, int * d_gate_end, int *d_port_start, int *d_port_end, long long int *d_M_EVENT_time, int *d_M_EVENT_val){
	...
	if(index < part_n){
		...
		if(part_cnt == 0){
			...
			long long int new_time = find_time(tmp_size, p1, p2, portfirst, portlast, d_M_EVENT_time);
			...
			for(int j = p1 ; j < p2 ; j++){
				int x = check_port_index(new_time, portfirst[j - p1], portlast[j - p1], d_M_EVENT_time);
...
__global__ void kernel2_find_partition_sizes(int thread_number, int part_n, int *d_out_pointer, int *d_gate_partition_number, long long int mx,int *d_partition_number, bool *d_zero_time, long long int *d_partition_min_time, int *d_stimuli_size, int *d_partition_pre, int *d_stimuli_sdf_first, int *d_stimuli_sdf_end, int *d_partition_address_first, int *d_partition_address_last, int *d_port_partition_start, int*d_port_partition_end, int *d_sdf_mask, int *d_sdf_s_in, int8_t *d_t_edge, int *d_table_id, long long int *d_tmp_memory_time, int *d_table, int *d_M_EVENT_val, long long int *d_M_EVENT_time){
	int index = blockIdx.x * blockDim.x + threadIdx.x;
	...
	if(index < part_n){
		...
		long long int min_partition_time = mx * 2;
		...
		while(true){
			...
			if(output_val != n_output_val){
				...
				long long int new_time = Time + delay_out;
...
__global__ void kernel2_2_find_partition_sizes(int part_n,int *d_out_pointer, int *d_gate_partition_number, long long int mx,int *d_partition_number, bool *d_zero_time, long long int *d_partition_min_time, int *d_stimuli_size, int *d_partition_pre, int *d_stimuli_sdf_first, int *d_stimuli_sdf_end, int *d_partition_address_first, int *d_partition_address_last, int *d_port_partition_start, int*d_port_partition_end, int8_t *d_t_edge, int *d_table_id, long long int *d_tmp_memory_time, int *d_table, int *d_M_EVENT_val, long long int *d_M_EVENT_time){
	int index = blockIdx.x * blockDim.x + threadIdx.x;
...
//kernel3
__global__ void kernel3_gpu_single_gate_simulation(int thread_number, int part_n, long long int mx,int *d_partition_number, int *d_partition_output_address, bool *d_zero_time, long long int *d_partition_min_time, int *d_stimuli_size, int *d_partition_pre, int *d_stimuli_sdf_first, int *d_stimuli_sdf_end, int *d_partition_address_first,
	 int *d_partition_address_last, int *d_port_partition_start, int *d_port_partition_end, int *d_sdf_mask, int *d_sdf_s_in, int8_t *d_t_edge, int *d_table_id, int *d_table, int *d_M_EVENT_val, long long int *d_M_EVENT_time, bool *d_fake_signal){
...
void tree_allocations(int LEVEL){
	int cnt_partition = 0;
	int cnt_gate = 0;
			...
			if(signall.name == "Fake"){
				...
				if(total_waveform_size <= 0){
					...
					for(int j = 0 ; j < compo.inputs.size(); j++){
						...
						kernel_print_waveform<<<1,1>>>(ptr1, ptr2, d_M_EVENT_val, d_M_EVENT_time);
...
__global__ void kernel4_make_saif(int part_n, long long int saif_min, long long int saif_mx, int *d_signal_pointer_first, int *d_signal_pointer_last, bool *d_signal_ending_part, bool *d_fake_part, long long int *d_saif_part, int *d_M_EVENT_val, long long int *d_M_EVENT_time){
	int index = blockIdx.x * blockDim.x + threadIdx.x;
	...
	if(index < part_n && fake_bool == false){
		...
		for(int i = pointer1 ; i < pointer2; i++){
			long long int t1 = max(saif_min, min(saif_mx, d_M_EVENT_time[i]));
			long long int t2 = max(saif_min, min(saif_mx, mx));
			if(i + 1 < pointer2){
				t2 = max(saif_min, min(saif_mx, d_M_EVENT_time[i + 1]));
			}
...
void saif_initialize(int LEVEL){
...
void gpusimultaion(){
	...
	input_init();
	cout<<"bam"<<endl;
	kernel0_input_init<<<((total_input_sig * sig_part_number + thread_number - 1) / thread_number), thread_number>>>(sig_part_number, total_input_sig, d_sig_input_M_pointer, d_sig_input_D_pointer, d_sig_input_size, d_input_time, d_input_val, d_M_EVENT_time, d_M_EVENT_val);
	input_free();
	...
	for(int i = 0; i < height.size(); i++){
		...
		cout<<"kernel4"<<endl;
		kernel4_make_saif<<<((total_number_of_signal_parts + thread_number - 1) / thread_number), thread_number>>>(total_number_of_signal_parts, MN, MX, d_signal_pointer_first, d_signal_pointer_last, d_signal_ending_part, d_fake_part, d_saif_part, d_M_EVENT_val, d_M_EVENT_time);
		cudaDeviceSynchronize();

		cudaMemcpy(h_saif_part, d_saif_part, total_number_of_signal_parts*3*sizeof(long long int),cudaMemcpyDeviceToHost);
		saif_process(i);
