...
void saif_start(long long int min,long long int max){
...
void saif_output(long long int ti,long long int to,long long int tx,string name){
  string s="";
  string t="";
...
void saif_process(long long int min,long long int max,string saif_name){
  fout = fopen (saif_name.c_str(),"w");
  saif_start(min,max);
  for(int i=3;i<signal_vector.size();i++){
    ...
    saif_output(sig.i_val, sig.o_val, sig.x_val,name);
    //fout<<s<<endl;
  }
  fprintf(fout,"))))");
  fclose(fout);
