...
void makec(){
  ...
  for(int i=0;i<m_name.size();i++)
    ch<<m_name[i]<<";"<<endl;
  //revert function
  ch<<"void revert()override;"<<endl;
  cc<<"void "<<module<<"::revert(){"<<endl;
  for(int i=0;i<p_name.size();i++){
    vector<string> com=get_all_word(erase_extra_space(p_name[i]));
    if(com[0]=="udp_dff" || com[0]=="udp_tlat"){
      cc<<com[1]<<".revert();"<<endl;
    }
  }
  cc<<"}"<<endl;
...
int main(){
  ...
  for(int i=0;i<all_module.size();i++){
    ...
    cc<<all_module[i]<<"* g1=new "<<all_module[i]<<"();"<<endl;
    //cc<<"cout<<sizeof("<<all_module[i]<<")<<endl;"<<endl;
    //cc<<"cout<<sizeof(GATE)<<endl;"<<endl;
    cc<<"g=g1;"<<endl;
    cc<<"}"<<endl;
  }
  cc<<"return g;"<<endl;
